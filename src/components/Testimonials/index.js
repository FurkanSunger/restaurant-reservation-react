    ...
    return (
        <TestimonialsWrapper>
            <div className="container">
                ...
                <Slider {...settings}>
                    {
                        props.restaurant.map(item => item.testimonials.map(testimonial => (
                            <TestimonailItem key={testimonial.id}>
                                <PrimaryText className="color-light">{testimonial.message}</PrimaryText>
                                <PrimaryText className="fw-italic color-white secondary-font fs-3">{testimonial.name}</PrimaryText>
                            </TestimonailItem>
                        )))
                    }
                </Slider>
